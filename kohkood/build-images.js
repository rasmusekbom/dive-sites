...
async function run() {
  ...
  for (const file of files) {
    ...
    for (const w of widths) {
      await image.clone().resize({ width: w }).webp({ quality: 78 }).toFile(path.join(CACHE, `${name}-${w}.webp`));
    }
  ...
  for (const file of fs.readdirSync(SRC).filter(f => !/\.(jpe?g|png)$/i.test(f))) fs.copyFileSync(path.join(SRC, file), path.join(CACHE, file));
