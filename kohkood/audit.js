...
for (const p of pages) {
  ...
  if (hl < 9) issues.push(['hreflang-missing', url, hl]);
...
for (const p of pages) { const u = '/' + path.relative(dist, path.dirname(p)).replace(/\\/g, '/'); const url = u === '/.' ? '/' : u + '/'; if (!smUrls.has(url) && !/404/.test(url)) issues.push(['not-in-sitemap', url]); }
